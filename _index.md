...
### **Instance Creators**  
[Instance creator methods]({{< relref "instance_creators" >}}) generate new quadrille instances derived from the original. These include **[clone()]({{< relref "clone" >}})** to create a shallow copy, **[row(row)]({{< relref "row" >}})** to extract a specific row, **[crop(row, col, width, height, wrap)]({{< relref "crop" >}})** to extract a rectangular region, and **[ring(row, col, dimension, wrap)]({{< relref "ring" >}})** a square neighborhood centered at a specified position — center cell included.
